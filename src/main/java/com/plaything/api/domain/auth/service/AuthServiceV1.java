...
public class AuthServiceV1 {
  ...
  @Transactional
  public LoginResult login(OAuth2Provider provider, String providerId, String transactionId,
      String fcmToken) {
    ...
    String token = jwtProvider.createToken(user.getLoginId(), user.getRole().toString(),
        60 * 60 * 1000L);
    Login login = loginSuccessHandler.handleSuccessFulLogin(user.getLoginId(), transactionId,
        LocalDate.now());
  ...
  public String loginForDevelop(LoginRequest loginRequest) {
    ...
    String token = jwtProvider.createToken(loginRequest.loginId(), user.get().getRole().toString(),
        60 * 60 * 1000L);
    return token;
  }
