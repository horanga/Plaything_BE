...
public class ProfileQueryServiceV1 {
  ...
  public void validateRegistration(ProfileRegistration registration, User user) {
    ...
    if (registration.primaryRole().equals(PrimaryRole.TOP) || registration.primaryRole()
        .equals(PrimaryRole.BOTTOM)) {
  ...
  public void validateUpdate(ProfileUpdate updateRequest, Profile profile) {
    ...
    if (updateRequest.primaryRole().equals(PrimaryRole.TOP) || updateRequest.primaryRole()
        .equals(PrimaryRole.BOTTOM)) {
      List<PersonalityTraitConstant> traits = profile.getPersonalityTrait().stream()
          .map(PersonalityTrait::getTrait).collect(Collectors.toList());
  ...
  public List<String> getHideList(String loginId) {
    ...
    List<ProfileHidePreference> list = profileHidePreferenceRepository.findBySettingUser_LoginId(
        loginId);
    if (list.isEmpty()) {
